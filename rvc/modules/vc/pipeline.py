...
class Pipeline(object):
    def __init__(self, tgt_sr, config) -> None:
    ...
    def get_f0(self, input_audio_path, x, p_len, f0_up_key, f0_method, filter_radius, inp_f0 = None) -> tuple:
    ...
    def vc(self, model, net_g, sid, audio0, pitch, pitchf, times, index, big_npy, index_rate, version, protect):  # ,file_index,file_big_npy
        ...
        with (torch.no_grad()):
            hasp = (pitch != None and pitchf != None)
    ...
    def pipeline(self, model, net_g, sid, audio, input_audio_path, times, f0_up_key, f0_method, file_index, index_rate, if_f0, filter_radius, tgt_sr, resample_sr, rms_mix_rate, version, protect, f0_file = None):
        if (file_index and file_index != "" and os.path.exists(file_index) and index_rate != 0): # and file_big_npy != "" and os.path.exists(file_big_npy)
            try:
        ...
        if (if_f0 == 1):
            audio_opt.append(self.vc(model, net_g, sid, audio_pad[t:], pitch[:, t // self.window :] if (t != None) else pitch, pitchf[:, t // self.window :] if (t != None) else pitchf, times, index, big_npy, index_rate, version, protect)[self.t_pad_tgt : -self.t_pad_tgt])
        else:
