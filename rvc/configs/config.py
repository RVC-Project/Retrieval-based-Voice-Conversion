...
version_config_list: list = [
    os.path.join(root, file) for root, _, files in os.walk(os.path.dirname(os.path.abspath(__file__))) for file in files if (file.endswith(".json"))
]
...
class Config:
    ...
    def __init__(self) -> None:
        ...
        (self.python_cmd, self.listen_port, self.noparallel, self.noautoopen, self.dml) = self.arg_parse()
        self.x_pad, self.x_query, self.x_center, self.x_max = self.device_config()

    @staticmethod
    def load_config_json() -> dict:
        return {
            config_file: json.load(open(config_file, "r")) for config_file in version_config_list
        }
    ...
    def arg_parse() -> tuple:
        ...
        return (cmd_opts.pycmd, cmd_opts.port, cmd_opts.noparallel, cmd_opts.noautoopen, cmd_opts.dml)

    @staticmethod
    def has_mps() -> bool:
        return torch.backends.mps.is_available() and not torch.zeros(1).to(torch.device("mps"))
    ...
    def device_config(self) -> tuple:
        if (torch.cuda.is_available() and self.use_gpu):
            ...
            if (("16" in self.gpu_name and "V100" not in self.gpu_name.upper()) or "P40" in self.gpu_name.upper() or "P10" in self.gpu_name.upper() or "1060" in self.gpu_name or "1070" in self.gpu_name or "1080" in self.gpu_name):
            ...
            logger.info("No supported Nvidia GPU found, using mps instead")
            ...
            logger.info("No supported Nvidia GPU found, using cpu instead")
